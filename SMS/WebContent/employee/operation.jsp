...
<body>
  ...
  <li ><a id="head">新增员工</a></li>
  <li><a id="cen">员工查找</a></li>
  <li><a id="foot">工资管理</a></li>
</ol> 
  <div class="context">
      <iframe id="frame" src="addemp.jsp"></iframe>
  </div>
   </div>
...
<script type="text/javascript">
  ...
  document.getElementById("head").onclick=function(){
		...
		changeone('#b9c6bd','none','none');
		document.getElementById("frame").src="addemp.jsp";
		
	 }
  document.getElementById("cen").onclick=function(){
	    var name=prompt("输入员工名：");
	    if(name!=null){
    	...
    	xmlHttpRequest.onreadystatechange=function(){
    		if(xmlHttpRequest.readyState==4 && xmlHttpRequest.status==200){
        		...
        		var data=xmlHttpRequest.responseText;
        		 if(name!=""){
        	    if(data==="true"){
        	    	changeone('none','#b9c6bd','none');
        	    	document.getElementById("frame").src="CheckempServlet?name="+name+"&id="+id;
        	    }else if(data==="false")
        	    	  alert("该员工不存在！");
        		 }else
         	    	alert("选项不能为空！");
  ...
  document.getElementById("foot").onclick=function(){
		changeone('none','none','#b9c6bd');
		document.getElementById("frame").src="salma.jsp";
}
		</script>
</body>
</html>